...
use crate::constants::{MIN_DEPOSIT_USDC, RAYDIUM_CLMM_PROGRAM_ID, WSOL_MINT};
...
pub fn handler(ctx: Context<DepositUsdc>, amount: u64) -> Result<()> {
    ...
    require!(amount >= MIN_DEPOSIT_USDC, VaultError::DepositTooSmall);
    ...
    // Read SOL price live from Raydium CLMM pool (needed for TVL calculation)
    let sol_price_usd = {
    ...
    // Calculate TVL on-chain (needed for correct share pricing)
    let current_tvl = vault.calculate_tvl(sol_price_usd);

    // USDC is already in USD (6 decimals)
    ...
    // Update vault state
    vault.treasury_usdc = vault.treasury_usdc
        .checked_add(amount)
    ...
    user_deposit.shares = user_deposit.shares
        .checked_add(shares_to_mint)
        .ok_or(error!(VaultError::MathOverflow))?;
    user_deposit.total_deposited_usdc = user_deposit.total_deposited_usdc
        .checked_add(amount)
        .ok_or(error!(VaultError::MathOverflow))?;
